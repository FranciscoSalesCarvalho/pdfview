...
import com.example.myapplication.ImageSource
import com.example.myapplication.SubsamplingScaleImageView
import com.example.myapplication.decoder.*
import com.example.myapplication.decoder.ImageDecoder
...
open class SubsamplingScaleImageView @JvmOverloads constructor(
    ...
    private var hasBaseLayerTiles = true

    // Map of zoom level to tile grid
    private var tileMap: MutableMap<Int, List<Tile>>? =
        null
    ...
    private val decoderLock: ReadWriteLock =
    ...
    private var regionDecoderFactory: DecoderFactory<out ImageRegionDecoder> =
        CompatDecoderFactory<ImageRegionDecoder>(
    ...
    fun setImage(
        ...
        if (previewSource != null) {
            ...
            if (previewSource.bitmap != null) {
                ...
                val task =
                    BitmapLoadTask(this, context, bitmapDecoderFactory, uri, true)
                execute(task)
            }
        }
        if (imageSource.bitmap != null && imageSource.sRegion != null) {
            ...
            if (imageSource.tile || sRegion != null) {
                ...
                val task =
                    TilesInitTask(this, context, regionDecoderFactory, uri)
                execute(task)
            } else {
                // Load the bitmap as a single image.
                val task =
                    BitmapLoadTask(this, context, bitmapDecoderFactory, uri, false)
                execute(task)
            }
        }
    }
    ...
    private fun setGestureDetector(context: Context) {
        detector = GestureDetector(context, object : SimpleOnGestureListener() {
            override fun onFling(
                ...
                if (panEnabled && isReady && vTranslate != null && e1 != null && e2 != null && (Math.abs(
                        e1.x - e2.x
                    ) > 50 || Math.abs(e1.y - e2.y) > 50) && (Math.abs(
                        velocityX
                    ) > 500 || Math.abs(velocityY) > 500) && !isZooming
                ) {
                    ...
                    AnimationBuilder(PointF(sCenterXEnd, sCenterYEnd)).withEasing(
                        EASE_OUT_QUAD
            ...
            override fun onDoubleTap(e: MotionEvent): Boolean {
                if (isZoomEnabled && isReady && vTranslate != null) {
                    ...
                    return if (isQuickScaleEnabled) {
                        ...
                        doubleTapZoom(
                            viewToSourceCoord(PointF(e.x, e.y)),
                            PointF(e.x, e.y)
                        )
                        true
                    }
                ...
                if (maxTouchCount > 0) {
                    if (touchCount >= 2) {
                        ...
                        if (isZoomEnabled && (distance(
                            ...
                            scale =
                                Math.min(maxScale, vDistEnd / vDistStart * scaleStart)
                            if (scale <= minScale()) {
                        ...
                        val spanDiff =
                            Math.abs(1 - dist / quickScaleLastDistance) * 0.5f
                        if (spanDiff > 0.03f || quickScaleMoved) {
                            ...
                            scale = Math.max(
                                minScale(),
                                Math.min(maxScale, scale * multiplier)
                            )
                            if (panEnabled) {
                                val vLeftStart = vCenterStart!!.x - vTranslateStart!!.x
                                val vTopStart = vCenterStart!!.y - vTranslateStart!!.y
    ...
    private fun doubleTapZoom(sCenter: PointF?, vFocus: PointF?) {
        ...
        val doubleTapZoomScale =
            Math.min(maxScale, doubleTapZoomScale)
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (tileMap != null && isBaseLayerReady) {
            ...
            val sampleSize =
                Math.min(fullImageSampleSize, calculateInSampleSize(scale))
                ...
                if (key == sampleSize || hasMissingTiles) {
                    for (tile in value) {
                        ...
                        if (!tile.loading && tile.bitmap != null) {
                            ...
                            if (requiredRotation == ORIENTATION_0) {
                                setMatrixArray(
                                    dstArray,
                            ...
                            } else if (requiredRotation == ORIENTATION_90) {
                                setMatrixArray(
                                    dstArray,
                                    tile.vRect.right.toFloat(),
                                    tile.vRect.top.toFloat(),
            ...
            matrix!!.postRotate(requiredRotation.toFloat())
            ...
            if (tileBgPaint != null) {
                ...
                sRect!![0f, 0f, if (bitmapIsPreview) bitmap!!.width.toFloat() else sWidth.toFloat()] =
                    if (bitmapIsPreview) bitmap!!.height.toFloat() else sHeight.toFloat()
    ...
    private fun initialiseBaseLayer(maxTileDimensions: Point) {
        ...
        if (fullImageSampleSize == 1 && sRegion == null && sWidth() < maxTileDimensions.x && sHeight() < maxTileDimensions.y) {
            ...
            if (hasBaseLayerTiles) {
                ...
                val task = TileLoadTask(this, decoder, baseGrid!![0])
                execute(task)
            }
        }
    }
    ...
    private fun calculateInSampleSize(scale: Float): Int {
        ...
        if (sHeight() > reqHeight || sWidth() > reqWidth) {
            ...
            val heightRatio =
                Math.round(sHeight().toFloat() / reqHeight.toFloat())
            val widthRatio =
                Math.round(sWidth().toFloat() / reqWidth.toFloat())
    ...
    private fun initialiseTileMap(maxTileDimensions: Point) {
        ...
        while (true) {
            ...
            val tileGrid: MutableList<Tile> =
                ArrayList(xTiles * yTiles)
    ...
    private class Anim {
        val scaleStart // Scale at start of anim = 0f
        val scaleEnd // Scale at end of anim (target) = 0f
        ...
        val interruptible =
            true // Whether the anim can be interrupted by a touch
        val easing =
            EASE_IN_OUT_QUAD // Easing style
        val origin =
            ORIGIN_ANIM // Animation origin (API, double tap or fling)
    ...
    fun viewToFileRect(vRect: Rect, fRect: Rect) {
        ...
        fRect[viewToSourceX(vRect.left.toFloat()).toInt(), viewToSourceY(vRect.top.toFloat()).toInt(), viewToSourceX(
            vRect.right.toFloat()
        ).toInt()] = viewToSourceY(vRect.bottom.toFloat()).toInt()
    ...
    private fun vTranslateForSCenter(
        ...
        val vxCenter =
            paddingLeft + (width - paddingRight - paddingLeft) / 2
        val vyCenter =
            paddingTop + (height - paddingBottom - paddingTop) / 2
        if (satTemp == null) {
            satTemp = ScaleAndTranslate(0, PointF(0, 0))
        }
        satTemp!!.scale = scale
    ...
    fun setBitmapDecoderClass(bitmapDecoderClass: Class<out ImageDecoder>) {
        ...
        bitmapDecoderFactory =
            CompatDecoderFactory(bitmapDecoderClass)
    }
    ...
    fun getPanRemaining(vTarget: RectF) {
        ...
        if (panLimit == PAN_LIMIT_CENTER) {
            ...
            vTarget.bottom =
                Math.max(0f, vTranslate!!.y - (height / 2 - scaleHeight))
     ...
     */
    fun setHasBaseLayerTiles(hasBaseLayerTiles: Boolean) {
    ...
    companion object {
        ...
        private val VALID_ZOOM_STYLES = Arrays.asList(
            ZOOM_FOCUS_FIXED,
            ZOOM_FOCUS_CENTER,
        ...
        private val VALID_EASING_STYLES =
            Arrays.asList(
                EASE_IN_OUT_QUAD,
                EASE_OUT_QUAD
            )
        ...
        private val VALID_PAN_LIMITS = Arrays.asList(
            PAN_LIMIT_INSIDE,
            PAN_LIMIT_OUTSIDE,
            PAN_LIMIT_CENTER
        )
    ...
    init {
        ...
        if (attr != null) {
            ...
            if (typedAttr.hasValue(R.styleable.SubsamplingScaleImageView_assetName)) {
                val assetName =
                    typedAttr.getString(R.styleable.SubsamplingScaleImageView_assetName)
            ...
            if (typedAttr.hasValue(R.styleable.SubsamplingScaleImageView_src)) {
                val resId =
                    typedAttr.getResourceId(R.styleable.SubsamplingScaleImageView_src, 0)
                if (resId > 0) {
            ...
            if (typedAttr.hasValue(R.styleable.SubsamplingScaleImageView_zoomEnabled)) {
                isZoomEnabled = typedAttr.getBoolean(
            ...
            if (typedAttr.hasValue(R.styleable.SubsamplingScaleImageView_quickScaleEnabled)) {
                isQuickScaleEnabled = typedAttr.getBoolean(
